...
async function run() {
  ...
  r = await fetch(`https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&playlistId=${pid}&maxResults=1&key=${key}`);
  d = await r.json();
  console.log(d.items[0].snippet.resourceId.videoId);
}
