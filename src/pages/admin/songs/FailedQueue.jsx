...
const FailedQueue = () => {
    ...
    const fetchFailed = async () => {
        try {
            ...
            if (res.data.success) setFailedImports(res.data.failed);
        } catch (err) {
            ...
            <div className="bg-rose-50 p-4 border-b border-rose-100 flex justify-between items-center">
                <div className="flex items-center gap-3 text-rose-700">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-rose-600 opacity-80">{failedImports.length} items require attention</p>
                    </div>
                </div>
                <button 
                    disabled={actionLoading}
                    onClick={() => {
                        const ids = failedImports.map(f => f._id);
                        // bulk retry not fully implemented in this UI yet, but could trigger retryAllFailed
                    }}
                    className="bg-white text-rose-700 border border-rose-200 px-4 py-2 rounded-lg font-medium hover:bg-rose-50 text-sm"
                >
                    Retry All
                </button>
            </div>
