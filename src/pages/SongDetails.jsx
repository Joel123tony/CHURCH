...
export default function SongDetails() {
  ...
  return (
    <div className={`min-h-screen font-sans pb-[calc(6rem+env(safe-area-inset-bottom))] relative transition-colors duration-500 overflow-x-hidden ${isDark ? 'bg-[#0f172a]' : 'bg-[#F4EFE7]'}`}>
      ...
      <div className={`sticky top-[var(--navbar-height)] z-40 backdrop-blur-md border-b shadow-sm transition-colors duration-500 ${isDark ? 'bg-[#1e293b]/95 border-gray-700/50' : 'bg-white/95 border-[#E8DCCB]'}`}>
        <div className="max-w-5xl mx-auto px-3 sm:px-4 py-3 sm:py-4 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3 md:gap-4">
          
          <div className="flex w-full md:w-auto items-center justify-start shrink-0">
            <Link
              to="/songs"
              className={`flex items-center justify-center gap-2 px-3 sm:px-4 py-2 rounded-xl font-bold text-sm border transition-all shadow-sm shrink-0 min-h-11 ${isDark ? 'bg-gray-800 text-gray-300 border-gray-700 hover:border-gray-600 hover:bg-gray-700' : 'bg-[#F4EFE7] text-[#54091b] border-[#54091b]/10 hover:border-[#54091b]/30 hover:bg-white'}`}
            >
              <ChevronLeft size={16} />
              <span>{t("Back")}</span>
            </Link>
          ...
          <div className="flex-1 w-full min-w-0 text-center flex flex-col justify-center px-1 sm:px-2">
            {songMeta.titleEnglish && (
               <div className={`text-sm md:text-base font-bold transition-colors whitespace-normal break-words leading-tight md:truncate md:whitespace-nowrap ${isDark ? 'text-gray-400' : 'text-slate-500'}`}>{songMeta.titleEnglish}</div>
            )}
            <div className={`text-base md:text-lg font-black transition-colors whitespace-normal break-words leading-tight md:truncate md:whitespace-nowrap ${isDark ? 'text-white' : 'text-[#54091b]'}`}>{songMeta.titleTamil || songMeta.title}</div>
          </div>

          <div className="w-full md:w-auto flex flex-col sm:flex-row items-stretch md:items-center justify-between md:justify-end gap-3 shrink-0">
            <div className="w-full md:w-auto flex-1 min-w-0 max-w-none md:max-w-[320px]">
              <SongInlineSearch />
            </div>
            <button
              onClick={toggleTheme}
              className={`p-2.5 rounded-xl transition-colors border-2 shrink-0 self-end sm:self-auto min-h-11 min-w-11 flex items-center justify-center ${isDark ? 'border-gray-700 bg-gray-800 text-yellow-400 hover:border-gray-600' : 'border-[#54091b]/10 bg-[#F4EFE7] text-[#54091b] hover:border-[#54091b]/30'}`}
              title="Toggle Theme"
            >
      ...
      <div className="max-w-4xl mx-auto px-3 sm:px-6 pt-4 sm:pt-6">
        ...
        <div className={`flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4 mb-6 sm:mb-8 animate-fade-in-up p-3 sm:p-4 rounded-[20px] border shadow-sm transition-colors duration-500 ${isDark ? 'bg-[#1e293b] border-gray-700' : 'bg-white border-[#E8DCCB]'}`}>
        ...
        {loading ? (
          ...
          /* Pending Fetch State */
          <div className={`rounded-[24px] shadow-sm border p-12 text-center animate-fade-in-up transition-colors duration-500 ${isDark ? 'bg-[#1e293b] border-gray-700' : 'bg-white border-[#E8DCCB]'}`}>
            <div className="w-20 h-20 flex items-center justify-center mx-auto mb-6">
              <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[#54091b]"></div>
            </div>
          ...
          /* Unavailable State */
          <div className={`rounded-[24px] shadow-sm border p-12 text-center animate-fade-in-up transition-colors duration-500 ${isDark ? 'bg-[#1e293b] border-gray-700' : 'bg-white border-[#E8DCCB]'}`}>
            <div className={`w-20 h-20 flex items-center justify-center mx-auto mb-6 rounded-full ${isDark ? 'bg-gray-800' : 'bg-[#F4EFE7]'}`}>
              <AlertCircle size={40} className="text-[#54091b]" />
            </div>
            <h2 className={`text-xl font-bold mb-4 ${isDark ? 'text-gray-200' : 'text-gray-800'}`}>
          ...
          /* Content Container */
          <div className={`rounded-[24px] sm:rounded-[32px] p-4 sm:p-12 shadow-sm border animate-fade-in-up transition-colors duration-500 overflow-hidden ${isDark ? 'bg-[#1e293b] border-gray-700' : 'bg-white border-[#E8DCCB]'}`}>
              ...
              <div className="flex justify-center mb-6 sm:mb-8 overflow-x-auto">
                 <div className={`p-1 rounded-2xl inline-flex shadow-inner transition-colors max-w-full ${isDark ? 'bg-gray-800' : 'bg-[#F4EFE7]'}`}>
                    <button 
                       ...
                       className={`px-4 sm:px-8 py-2 sm:py-2.5 rounded-xl font-bold transition-all text-sm sm:text-base whitespace-nowrap ${activeTab === 'tamil' ? (isDark ? 'bg-gray-700 text-[#D4AF37] shadow-sm' : 'bg-white text-[#54091b] shadow-sm') : (isDark ? 'text-gray-400 hover:text-gray-200' : 'text-slate-500 hover:text-slate-700')}`}
                    >
                       Tamil Lyrics
                    </button>
                    <button 
                       onClick={() => setActiveTab('english')}
                       className={`px-4 sm:px-8 py-2 sm:py-2.5 rounded-xl font-bold transition-all text-sm sm:text-base whitespace-nowrap ${activeTab === 'english' ? (isDark ? 'bg-gray-700 text-[#D4AF37] shadow-sm' : 'bg-white text-[#54091b] shadow-sm') : (isDark ? 'text-gray-400 hover:text-gray-200' : 'text-slate-500 hover:text-slate-700')}`}
                    >
            ...
            {/* Lyrics Area */}
            <div className="prose prose-lg max-w-none text-center mx-auto transition-all duration-300 ease-out overflow-x-hidden" style={{ fontSize: `${1.125 * (zoomLevel / 100)}rem` }}>
              {activeTab === 'tamil' && songSections && songSections.length > 0 ? (
                <div className={`font-medium leading-[1.95] sm:leading-[2.5] whitespace-pre-wrap break-words [overflow-wrap:anywhere] font-serif tracking-[0.01em] transition-colors ${isDark ? 'text-gray-300' : 'text-[#1E293B]'}`}>
                  {songSections.map((section, idx) => (
                    <div key={idx} className="mb-10 animate-fade-in-up" style={{ animationDelay: `${idx * 100}ms` }}>
                      {(section.label || section.type) && (
                         <div className={`text-sm sm:text-base font-bold mb-3 uppercase tracking-wider ${isDark ? 'text-[#D4AF37]' : 'text-[#54091b]'}`}>
                           {section.label || `${section.type} ${section.number || ''}`.trim()}
                         </div>
                      )}
                      <div>
                        {(section.lines || []).map((line, lIdx) => (
                          <div key={lIdx} className="min-h-[1.5rem]">{line}</div>
                        ))}
                      </div>
                    </div>
                  ))}
                ...
                <div
                  className={`font-medium leading-[1.95] sm:leading-[2.5] whitespace-pre-wrap break-words [overflow-wrap:anywhere] font-serif tracking-[0.01em] transition-colors ${isDark ? 'text-gray-300' : 'text-[#1E293B]'}`}
